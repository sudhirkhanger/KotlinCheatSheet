...
#+begin_src java
val numbersMap = mapOf("key1" to 1, "key2" to 2, "key3" to 3, "key4" to 1) // creates an immutable map without write operations
...
#+begin_src java
val numbersMap = mutableMapOf("key1" to 1, "key2" to 2, "key3" to 3) // creates an immutable map without write operations
